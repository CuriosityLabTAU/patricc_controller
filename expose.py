...
roslib.load_manifest("dynamixel_hr_ros", bootstrap_version="1.0")
import rospy
...
class DxlROS(Thread):
    ...
    def create_publisher(self, topic, type):
        ...
        self.pub[topic] = rospy.Publisher(topic, type, queue_size=10)
    ...
    def publish(self):
        ...
        for id in self.motors:
            ...
            #speed.append(s)
            #t = self.chain.get_reg_si(id,"present_temp")
            #temp.append(t)
            #l = self.chain.get_reg_si(id,"present_load")
            #torque.append(l)
        ...
        msg.id = self.motors
        msg.angle = angle
        #msg.speed = angle #speed
        #msg.temp = angle #temp
        #msg.torque = angle #torque
        # ~ msg.moving=moving
...
if __name__ == "__main__":
    ...
    parser.add_argument("--baudrate", type=int, help="Baudrate to use on the serial device", default=1000000)
    parser.add_argument("--rate", type=int, help="Publishing rate for ROS", default=50)
    args = parser.parse_args()

    # because of dynamic allocation of USB dirs
    # go over several options until you find the correct one
    chain = None
    for i in range(5):
        usb_dir = "/dev/ttyUSB%s" % str(i)
        logging.info("Connecting to chain on device %s at rate %d" % (usb_dir, args.baudrate))
        try:
            chain = dxlchain.DxlChain(usb_dir, rate=args.baudrate, timeout=0.5)
            print('robot on %s' % usb_dir)
            break
    ...
    if chain:
        ...
        logging.error("Chain not detected!")
